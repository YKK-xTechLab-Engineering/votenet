...
from votenet.sunrgbd import sunrgbd_utils
from votenet.sunrgbd.model_util_sunrgbd import SunrgbdDatasetConfig
...
class SunrgbdDetectionVotesDataset(Dataset):
    def __init__(self, dirpath:Path | str="data", split_set='train', num_points=20000,
        ...
        self.use_v1 = use_v1 
        if use_v1:
            self.data_path = os.path.join(str(dirpath),
                'sunrgbd/sunrgbd_pc_bbox_votes_50k_v1_%s'%(split_set))
        else:
            self.data_path = os.path.join(str(dirpath),
                'sunrgbd/sunrgbd_pc_bbox_votes_50k_v2_%s'%(split_set))

        self.raw_data_path = os.path.join(str(dirpath), 'sunrgbd/sunrgbd_trainval')
