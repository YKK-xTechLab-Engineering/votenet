...
from votenet.utils import pc_util
from votenet.sunrgbd.sunrgbd_utils import sunrgbd_utils
...
def data_viz(data_dir, dump_dir="data/data_viz_dump"):  
...
if __name__=='__main__':
    ...
    SUNRGBD_DIR = DATA_DIR / "sunrgbd_trainval"
    if args.viz:
        data_viz(str(SUNRGBD_DIR))
        exit()

    if args.compute_median_size:
        get_box3d_dim_statistics(str(SUNRGBD_DIR / "train_data_idx.txt"))
        exit()

    if args.gen_v1_data:
        extract_sunrgbd_data(str(SUNRGBD_DIR / "train_data_idx.txt"),
            split = 'training',
            output_folder = str(DATA_DIR / "sunrgbd_pc_bbox_votes_50k_v1_train"),
            save_votes=True, num_point=50000, use_v1=True, skip_empty_scene=False)
        extract_sunrgbd_data(str(SUNRGBD_DIR / "val_data_idx.txt"),
            split = 'training',
            output_folder = str(DATA_DIR / "sunrgbd_pc_bbox_votes_50k_v1_val"),
            save_votes=True, num_point=50000, use_v1=True, skip_empty_scene=False)
    
    if args.gen_v2_data:
        extract_sunrgbd_data(str(SUNRGBD_DIR / "train_data_idx.txt"),
            split = 'training',
            output_folder = str(DATA_DIR / "sunrgbd_pc_bbox_votes_50k_v2_train"),
            save_votes=True, num_point=50000, use_v1=False, skip_empty_scene=False)
        extract_sunrgbd_data(str(SUNRGBD_DIR / "val_data_idx.txt"),
            split = 'training',
            output_folder = str(DATA_DIR / "sunrgbd_pc_bbox_votes_50k_v2_val"),
            save_votes=True, num_point=50000, use_v1=False, skip_empty_scene=False)
