...
from votenet.scannet.model_util_scannet import rotate_aligned_boxes
from votenet.scannet.model_util_scannet import ScannetDatasetConfig
...
SCANNET_DIR = CURRENT_FILEPATH.parent
META_DATA_DIR = SCANNET_DIR / 'meta_data'

class ScannetDetectionDataset(Dataset):
       
    def __init__(self, 
                 dirpath:Path | str="data",
                 split_set='train', 
                 num_points=20000,
                 use_color=False, 
                 use_height=False, 
        ...
        self.data_path = str(dirpath / 'scannet_train_detection_data')
        all_scan_names = list(set([os.path.basename(x)[0:12] \
            for x in os.listdir(self.data_path) if x.startswith('scene')]))
        if split_set=='all':            
            self.scan_names = all_scan_names
        elif split_set in ['train', 'val', 'test']:
            split_filenames = os.path.join(str(META_DATA_DIR), 'scannetv2_{}.txt'.format(split_set))
...
if __name__=='__main__': 
    dset = ScannetDetectionDataset(dirpath="data",use_height=True, num_points=40000)
